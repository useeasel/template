...
export interface DesignTokens {
  ...
  pages: { about: boolean; contact: boolean; cv: boolean; press: boolean; exhibitions: boolean; news: boolean; available: boolean; presskit: boolean; commissions: boolean };
}
...
export const DEFAULT_DESIGN: DesignTokens = {
  ...
  pages: { about: true, contact: true, cv: true, press: true, exhibitions: false, news: false, available: false, presskit: false, commissions: false },
};
...
export function designClasses(d: DesignTokens): string {
  const pages = ['about', 'contact', 'cv', 'press', 'exhibitions', 'news', 'available', 'presskit', 'commissions'] as const;
  return [
    `ez-nav-${d.nav.layout}`,
